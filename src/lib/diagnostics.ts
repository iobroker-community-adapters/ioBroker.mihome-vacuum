const sensitiveKeyPattern =
    /(?:authorization|bssid|cookie|deviceid|did|homeid|ip|location|loginurl|lp|mac|nonce|password|security|secret|session|ssid|token|uid|url)/i;
const safeStringKeys = new Set(['command', 'manager', 'method', 'model', 'operation', 'region', 'status']);

function summarizeString(key: string, value: string): string {
...
export function logAdvancedDiagnostic(
