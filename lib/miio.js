...
class Miio extends EventEmitter {
	...
	constructor(adapterInstance) {
		...
		this.server.on('message', (msg, rinfo) => {
			if (rinfo.port === parseInt(this.port)) {
				if (msg.length === 32) {
					//clear Timepout
					clearTimeout(this.timeout);
					this.timeout = null;
     ...
     * @return {object} return a obj with the answer from the robot
     */
     ...
     */
	 async miotGet(ssid, piid, params) {
		return new Promise((resolve, reject) => {
			...
			const msgCounter = this.packet.msgCounter++;
			const rawMsg = this.packet.getRaw_fast(this._buildMsg('get_properties', [{did:'',siid:ssid, piid: piid}], msgCounter));
			let timeout = null;
