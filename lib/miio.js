...
class Miio extends EventEmitter {
	...
	async sendMessage(method, params) {
		return new Promise((resolve, reject) => {
			...
			if (!this.connected){
				adapter.log.warn('your device is not connected, but this could be temporary');
				return resolve({});
			}
				...
				if (msg.length !== 32 && rinfo.port === parseInt(this.port)) {
					...
					try {
						...
						try {
							...
							reject('MIIO MESSAGE CANT PARSE ANSWER: ' + plainData);
						} catch (error) {
							reject('MIIO MESSAGE CANT PARSE ANSWER: ' + error.message);
						}
					}
				}
			};
			...
			try {
				this.server.send(rawMsg, 0, rawMsg.length, parseInt(this.port), adapter.config.ip, err => {
					...
					this.globalTimeouts['sendMessage' + msgCounter] = setTimeout(cnt => {
						...
						this.server.removeListener('message', checkAnswer);
						adapter.log.warn('your device is connected, but didn\'t answer yet - sometime connection is broken and can take up to 10 Minutes');
						resolve({});
					}, 2000, msgCounter);
				});
			} catch (err) {
