...
class RoomManager {
		...
		adapter.createChannel('rooms', roomId, (err, roomObj) => {
			if (roomObj) {
				...
				adapter.setObjectNotExists(
					...
					(err) =>
						!err && adapter.setState(roomObj.id + '.mapIndex', mapIndex, true)
				);
	...
	updateRoomStates(roomObj_id) {
		...
		adapter.getObject('control.water_box_mode', (err, obj) => {
			obj && adapter.getState(obj._id, (err, comonState) => {
				adapter.setObject(roomObj_id + '.roomWaterBoxMode', {
					type: 'state',
					common: obj.common,
					native: {}
				},
		...
		adapter.getObject('control.water_box_level', (err, obj) => {
			obj && adapter.getState(obj._id, (err, comonState) => {
				adapter.setObjectNotExists(roomObj_id + '.roomWaterBoxLevel', {
					type: 'state',
					common: obj.common,
					native: {}
				},
