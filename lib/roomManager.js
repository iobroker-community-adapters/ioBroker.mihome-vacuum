...
class RoomManager {
    constructor(adapterInstance, i18nInstance) {
        ...
        adapter.getStates(adapter.namespace + '.rooms.*.roomClean', function (err, states) {
            if (states)
                for ( let stateId in states){
                    adapter.setObjectNotExists(stateId.replace(".roomClean", ".state"), roomManager.stateRoomStatus, function (err, obj) {
                        obj && adapter.setForeignState(obj.id, '', true);
                    });
                }
        });
    }
        ...
        adapter.getChannelsOf("rooms", function(err, roomObjs) {
            for (let r in roomObjs) {
                ...
                if (extRoomId.indexOf("manual_") == -1) {
                    ...
                    if (!room) {
                        adapter.setStateChanged(roomObj._id + ".mapIndex", i18n.notAvailable, true, function(err,id,notChanged){
                            if (!notChanged){
                                adapter.log.info("room: " + extRoomId + " not mapped");
                                adapter.delObject(roomObj._id + ".roomClean");
                                adapter.delObject(roomObj._id + ".state");
                            }
                        });
                    } else {
                        adapter.setStateChanged(roomObj._id + ".mapIndex", room, true, function(err,id,notChanged){
                            if (!notChanged){
                                adapter.log.info("room: " + extRoomId + " mapped with index " + room)
                                adapter.setObjectNotExists(id + ".roomClean", roomManager.stateRoomClean);
                                adapter.setObjectNotExists(id + ".state", roomManager.stateRoomStatus);
                            }
                        });
    ...
    findChannelsByMapIndex(mapList, callback) {
        adapter.getStates(adapter.namespace + '.rooms.*.mapIndex', function (err, states) {
            if (states){
                let channels= [];
                for ( let stateId in states){
