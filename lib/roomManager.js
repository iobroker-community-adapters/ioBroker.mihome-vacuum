            ...
            if (roomObj) {
                ...
                adapter.getObject('control.fan_power', (err, obj) => {
                    obj && adapter.getState(obj._id, (err, comonState) => {
                        ...
                        err => !err && comonState && adapter.setState(roomObj.id + '.roomFanPower', comonState.val, false));
                    });
                });
                adapter.getObject('control.water_box_mode', (err, obj) => {
                    obj && adapter.getState(obj._id, (err, comonState) => {
                        ...
                        err => !err && comonState && adapter.setState(roomObj.id + '.roomWaterBoxMode', comonState.val, false));
                    });
                });
                adapter.getObject('control.mop_mode', (err, obj) => {
                    obj && adapter.getState(obj._id, (err, comonState) => {
                        adapter.setObjectNotExists(roomObj.id + '.roomMopMode', {
                            type: 'state',
                            common: obj.common,
                            native: {}
                        },
                        err => !err && comonState && adapter.setState(roomObj.id + '.roomMopMode', comonState.val, false));
                    });
                });
            }
        });
