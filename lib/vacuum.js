...
class VacuumManager {
			...
			.then(async data => {
				if (data) {
					...
					if (Date.now() - this.mapLastSave > this.mapSaveIntervall) {
						...
						adapter.writeFile(`mihome-vacuum.${adapter.instance}.userfiles`, `actualMap.png`, buf, error => {
							if (error) {
	...
	async setGetStatus() {
		try {
			...
			this.features.water_box_mode == 2 && adapter.setStateAsync('control.water_box_level', {
	...
	async stateChange(id, state) {
		...
		try {
            ...
            activeCleanStates[this.cleanActiveState].resume
					) {
						if (state.val == true) {
							...
							if (this.cleandState === cleanStates.Pause) {
								await this.Miio.sendMessage(
									activeCleanStates[this.cleanActiveState].resume
								);
							} else await this.Miio.sendMessage('app_pause');
							adapter.setState(id, false, true);
						}
					} else {
						adapter.log.error(
							`Could not pause or Resume, because no cleaning active`
						);
					}
					break;
				...
				case 'water_box_mode':
					if (state.val != 207){
						...
						id = id.replace('.water_box_mode', '.water_box_level');
						state.val = adapter.getForeignState(id).val;
					}
				// eslint-disable-next-line no-fallthrough
				case 'water_box_level':
					await this.Miio.sendMessage('set_water_box_custom_mode', {'water_box_mode': 207, 'distance_off': 210 - state.val * 5});
					adapter.setForeignState(id, state.val, true);
					break;
					...
					if (commands[command]) {
						...
						if (parent === 'timer') {
							adapter.setForeignState(
								id,
								state.val == TimerManager.SKIP ||
                  state.val == TimerManager.DISABLED
									? state.val
			...
			switch (obj.command) {
				...
				case 'cleanZone':
					...
					if (await this.startCleaning(cleanStates.ZoneCleaning, obj)) {
						if (obj.repeat){ // would be set, if we only have one zone
				...
				case 'cleanSegments':
					...
					if (await this.startCleaning(cleanStates.RoomCleaning, obj)) {
						...
						if (answer.error) { // {"error":{"code":-10000,"message":"data for segment is not a number"}}
							if (params[0].repeat){ // some devices doesent support complex Object for app_segment_clean, so we have to use fallback mode
								...
								adapter.log.info('repeat will not supported native, so we use Queue as Fallback in future!');
							}
						}
						if (repeat) { // Falback mode
				...
				case 'setWaterBoxMode':
					...
					if (obj.message.waterBoxMode == 207){
						...
						return await this.Miio.sendMessage('set_water_box_custom_mode', {'water_box_mode': 207, 'distance_off': obj.message.waterBoxLevel});
					}
		...
		if (this.cleanActiveState) {
			...
			if (this.activeChannels && this.activeChannels.length === 1) {
				if (!messageObj.fanSpeed) {
					adapter.getState(this.activeChannels[0] + '.roomFanPower', (err, fanPower) =>
						fanPower && adapter.setStateChanged('control.fan_power', fanPower.val));
				}
				if (this.features.water_box_mode != null){
					if (this.features.water_box_mode == 2 && !messageObj.waterBoxLevel) {
						adapter.getState(this.activeChannels[0] + '.roomWaterBoxLevel',	(err, waterBoxLevel) =>
							waterBoxLevel &&  adapter.setStateChanged('control.water_box_level', waterBoxLevel.val, true));
					}
					if (!messageObj.waterBoxMode) {
						adapter.getState(this.activeChannels[0] + '.roomWaterBoxMode', (err, waterBoxMode) =>
							waterBoxMode && adapter.setStateChanged('control.water_box_mode', waterBoxMode.val));
					}
				}
				if (this.features.mop_mode != null && !messageObj.mopMode) {
					adapter.getState(this.activeChannels[0] + '.roomMopMode', (err, mopMode) =>
						mopMode && adapter.setStateChanged('control.mop_mode', mopMode.val));
				}
			...
			if (this.features.water_box_mode != null){
				if (messageObj.waterBoxLevel && this.features.water_box_mode == 2) {
					adapter.setState('control.water_box_level', messageObj.waterBoxLevel);
				} else if (messageObj.waterBoxMode) {
...
class FeatureManager {
	...
	init() {
		...
		adapter.getState(
			'info.device_model',
			(err, state) => state && state.val && this.setModel(state.val)
		);
			...
			adapter.getStates('rooms.*.roomFanPower', (err, states) => {
				if (states) {
					for (const stateId in states) {
						adapter.log.info(
							'change states from State control.fan_power ' + stateId
						);
	...
	async setWaterFilter() {
		if (this.water_filter === null) {
			...
			await Promise.all(
				objects.water_filter.map(async (o) => {
					const contents = await adapter.setObjectNotExistsAsync(
						'consumable.' + o._id,
						o
					);
					adapter.log.debug(
						'Create State for water_filter ' + JSON.stringify(contents)
					);
				})
			);
		}
