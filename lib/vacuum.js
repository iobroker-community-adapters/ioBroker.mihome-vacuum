...
class VacuumManager {
	...
	async getSetNetwork(){
		try {
			...
			if (answer.result !== 'unknown_method' && answer.result.rssi) {
	...
	async getMultiMapsList(){
		...
		try {
			...
			if (answer.result !== 'unknown_method') {
				let maps = answer.result[0].map_info
				...
				if(maps.length >0){
					...
					adapter.extendObjectAsync('cleanmap.actualMap',{common:{states:stateArray}})
					return true
				}
				return true
			}
			...
			switch (command) {
				...
				case 'carpet_mode':
					...
					if (state.val === true || state.val === 'true') {
						await this.Miio.sendMessage('set_carpet_mode', [{enable: 1}]);
						adapter.setForeignState(id, state.val, true);
					} else {
						await this.Miio.sendMessage('set_carpet_mode', [{enable: 0}]);
					...
					if (commands[command]) {
						...
						if (state.val !== false && state.val !== 'false') {
							...
							if(commands[command].method === 'reset_consumable'){
								this.globalTimeouts['onMessage'] = setTimeout(this.setGetConsumable, 500);
							}
						}
