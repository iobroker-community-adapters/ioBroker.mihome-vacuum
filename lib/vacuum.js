				...
				case 'cleanZone':
					...
					if (await this.startCleaning(cleanStates.ZoneCleaning, obj)) {
						if (obj.repeat){ // would be set, if we only have one zone
							adapter.log.error("repeat set" + JSON.stringify(obj.zones));
							obj.zones[0]= obj.zones[0].replace(/,[0-9]+\]/,',' + obj.repeat + ']');
							adapter.log.error("repeat set after" + JSON.stringify(obj.zones));
						}
				...
				case 'cleanSegments':
					...
					if (await this.startCleaning(cleanStates.RoomCleaning, obj)) {
						...
						if (obj.repeat){
							...
							params= obj.segments;
						return await this.Miio.sendMessage('app_segment_clean', params);
					}
	...
	async startCleaning(cleanStatus, messageObj) {
		...
		if (this.cleanActiveState) {
			...
			if (this.activeChannels && this.activeChannels.length === 1) {
				...
				adapter.log.error("repeat found" + JSON.stringify(repeatObj));
				if (repeatObj && !isNaN(repeatObj.val) && repeatObj.val > 0)
