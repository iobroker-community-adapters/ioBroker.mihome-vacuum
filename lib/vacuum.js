		...
		try {
            ...
            activeCleanStates[this.cleanActiveState].resume
				...
				case 'water_box_level':
					await this.Miio.sendMessage('set_water_box_distance_off', {'distance_off': 210 - state.val * 5});
					adapter.setForeignState(id, state.val, true);
					break;
				...
				case 'setWaterBoxMode':
					...
					if (obj.message.waterBoxMode == 207){
						if (requireParams('waterBoxLevel')) {
							this.Miio.sendMessage('set_water_box_distance_off', {'distance_off': obj.message.waterBoxLevel});
						}
		...
		if (this.cleanActiveState) {
			...
			if (this.activeChannels && this.activeChannels.length === 1) {
				...
				if (typeof messageObj.repeat === "undefined") {
					...
					if (repeatObj && Number(repeatObj.val) > 1) messageObj.repeat = repeatObj.val;
				}
			}
			if (messageObj.fanSpeed) {
