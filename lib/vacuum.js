...
class VacuumManager {
	constructor(adapterInstance, Miio) {
		...
		this.logEntries = [];
		this.cleanLog = [];
 		this.cleanLogHtmlAllLines = '';
		this.clean_log_html_table = '';
		...
		try {
			...
			const start = async () => {
				await this.asyncForEach(logarray, async (num) => {
					let ans = await this.Miio.sendMessage('get_clean_record', [num])
					adapter.log.warn('finish logs?' + JSON.stringify(ans))
				});
				adapter.log.warn('finish logs all?');
			  }
