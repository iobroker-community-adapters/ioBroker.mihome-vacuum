		...
		try {
			...
			if (this.mapGet) {
				...
				this.globalTimeouts['getMapData'] = setTimeout(async () => {
					...
					adapter.config.enableMiMap && this.getMapPointer(); // get pointer only by mimap
		...
		this.miApi.updateMap(this.mapPointer)
			.then(async data => {
				if (data) {
					...
					const rooms = data[1];
					if (this.device === 'roborock.vacuum.s5' && this.features.roomMapping === false && typeof rooms !== 'undefined' && rooms.length > 0) {
						const roomids = [];
