...
class VacuumManager {
	...
	async getStates() {
		...
		try {
			...
			NoError = true;
			adapter.log.debug('First run: ' + adapter.config.valetudo_enable + 'mimap: ' + adapter.config.enableMiMap + ' mapgget: '+ this.mapGet);
			adapter.log.debug('First timediff?: ' + (Date.now() - this.cMapLastPoll > this.cMapPoll));
	...
	async getMapPointer() {
		const trys = 0;
		adapter.log.debug('we need a Mappointer. Vaetudo: ' + adapter.config.valetudo_enable + 'mimap: ' + adapter.config.enableMiMap);
