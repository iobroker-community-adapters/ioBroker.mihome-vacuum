				...
				case 'cleanSegments':
					...
					if (await this.startCleaning(cleanStates.RoomCleaning, obj)) {
						...
						if (repeat){
							...
							} else if (!adapter.config.unsupported_segemntCleanRepeat){
								params= [{ 
									se_gments: obj.segments, 
									repeat: repeat
								}];
									// clean_order_mode': 0,
									// clean_mop: 0		
						...
						if (answer.error) { // {"error":{"code":-10000,"message":"data for segment is not a number"}}
							if (params[0].repeat){ // some devices doesent support complex Object for app_segment_clean, so we have to use fallback mode
								...
								answer = await this.Miio.sendMessage('app_segment_clean', params[0].se_gments);
								adapter.updateConfig({unsupported_segemntCleanRepeat: true}); // we will store this for future
