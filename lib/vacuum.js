...
class VacuumManager {
	...
	async getMultiMapsList(){
		...
		try {
			...
			if (answer && answer.result !== 'unknown_method') {
	...
	async checkFeaturesRoomMapping() {
		try {
			...
			if ( answer && answer.result !== 'unknown_method' && answer.result.length) {
	...
	async setGetCarpetMode() {
		try {
			const answer = await this.Miio.sendMessage('get_carpet_mode');
			if (answer && (answer.result[0] === 0 || answer.result[0] === 1)) {
