...
class VacuumManager {
	...
	async init() {
		...
		await Promise.all(
			objects.stockConsumable.map(async (o) => {
	...
	async setGetConsumable() {
		try {
			...
			this.features.water_filter &&
				adapter.setStateAsync("consumable.water_filter", {
	...
	async stateChange(id, state) {
		...
		try {
			switch (command) {
				...
				case "addRoom":
					if (!isNaN(state.val)) this.roomManager.createRoom("manual_" + state.val, parseInt(state.val, 10));
					else {
		...
		if (obj) {
			...
			switch (obj.command) {
					...
					if (!obj.zones) {
						...
						if (message.zones) {
							...
							if (message.hasOwnProperty("coordinates")) {
								if (message.hasOwnProperty("waterBoxMode")) obj.waterBoxMode = message.waterBoxMode;
								if (message.hasOwnProperty("waterBoxLevel")) obj.waterBoxLevel = message.waterBoxLevel;
								if (message.hasOwnProperty("mopMode")) obj.mopMode = message.mopMode;
								if (message.hasOwnProperty("fanSpeed")) obj.fanSpeed = message.fanSpeed;
								obj.zones = [message.coordinates];
							} else obj.zones = [obj.message];
						}
					}
				...
				case "cleanSegments":
					...
					if (!obj.segments) {
						...
						if (message.segments) {
							...
							if (typeof message == "object" && message.hasOwnProperty("rooms")) {
								...
								if (message.hasOwnProperty("mopMode")) obj.mopMode = message.mopMode;
								if (message.hasOwnProperty("fanSpeed")) obj.fanSpeed = message.fanSpeed;
								if (message.hasOwnProperty("repeat")) obj.repeat = message.repeat;
					...
					if (await this.startCleaning(cleanStates.RoomCleaning, obj)) {
						...
						if (repeat) {
							...
							for (let i = 1; i < repeat; i++) this.push(JSON.parse(JSON.stringify(obj)));
						}
						return answer;
					}
				...
				case "setWaterBoxMode":
					...
					} else return await this.Miio.sendMessage("set_water_box_custom_mode", [obj.message.waterBoxMode]);
