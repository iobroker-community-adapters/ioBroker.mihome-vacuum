...
class VacuumManager {
	...
	async setGetCarpetMode() {
		try {
			const answer = await this.Miio.sendMessage('get_carpet_mode');
			if (answer && (answer.result[0].enable === 0 || answer.result[0].enable === 1)) {
			...
			switch (obj.command) {
				...
				case 'cleanRooms':
					...
					if (!rooms || typeof rooms != "string") {
						return adapter.log.warn('cleanRooms needs as parameter a string of ioBroker room-id\'s');
					}
	...
	async setDustCollect(dust_collection_status) {
		if (this.dustCollect === null) {
			...
			if (this.dustCollect) {
				...
				adapter.log.info('dust collecting not found ' + JSON.stringify(dust_collection_status));
		}
	}	
