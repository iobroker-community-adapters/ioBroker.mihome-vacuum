			...
			.then(async data => {
				if (data) {
					...
					if (Date.now() - this.mapLastSave > this.mapSaveIntervall) {
						...
						adapter.writeFile(`mihome-vacuum.${adapter.instance}`, `actualMap.png`, buf, error => {
							if (error) {
								adapter.log.error('Error by saving of the map');
							} else {
								adapter.setState('cleanmap.mapURL', `/mihome-vacuum.${adapter.instance}/actualMap.png`, true);
							}
							this.mapLastSave = Date.now();
						});
					}
