...
class VacuumManager {
			...
			.then(async data => {
				...
				if (this.mapGet) {
					...
					this.globalTimeouts['getMapData'] = setTimeout(async () => {
						adapter.log.warn('Get Mappinter while cleaning');
						adapter.config.enableMiMap && await this.getMapPointer() // get pointer only by mimap
						adapter.log.warn('Get Mappinter done redomap...');
						//this.getMapData();
	...
	async setGetStatus() {
		try {
			...
			if (this.cleanState != status.state) {
	...
	async stateChange(id, state) {
		...
		try {
			switch (command) {
				...
				case 'start':
					if (state.val) {
						if (this.startCleaning(cleanStates.Cleaning, null)) {
	...
	async stopCleaning() {
		try {
			...
			this.clearQueue();
			this.cleanState = cleanStates.Unknown; // Force calling setRemoteState on next get_status answer
