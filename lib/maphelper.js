            ...
            req.on('response', function (res) {
                ...
                if (res.statusCode !== 200) {
                    if (res.statusCode === 404 || res.statusCode === '404') {
                        callback(`wrong server selected___${JSON.stringify(res)}`, null);
                    } else {
                        callback(`no map found on server___${JSON.stringify(res)}`, null);
                    }
                    return;
                }
                if (res.statusCode === 404 || res.statusCode === '404') {
                    adapter.log.debug(`Mapresponse_ ${JSON.stringify(res)}`);
                }
                                ...
                                if (!error && response && response.statusCode === 200) {
                                    ...
                                    if (error || !response) {
                                        this.adapter.log.error(
                                            `HTTP error when getting token cookie${JSON.stringify(error)}`,
                                        );
                                    } else {
                                        this.adapter.log.error(
                                            `HTTP error when getting token cookie code: ${response.statusCode} json_:${JSON.stringify(response)}`,
                                        );
                                    }
            ...
            this.login(username, password)
                .then(() => {
                    ...
                    const headers = {
                        ...
                        'User-Agent':
                            'Android-7.1.1-1.0.0-ONEPLUS A3010-136-9D28921C354D7 APP/xiaomi.smarthome APPV/62830',
                    ...
                    adapter.log.debug(`GET devices from:${url}`);
...
function generateSignature(path, params) {
    ...
    if (useRC4) {
        ...
        params.forEach(element => (body[element.key] = element.value));
        if (!body.data) {
            body['data'] = '';
        }
        /* body = {
        ...
        params.forEach(element => paramsTreeMap.set(element.key, element.value));
        ...
        if (paramsTreeMap.getLength() > 0) {
            paramsTreeMap.each((value, key) => paramsArray.push(`${key}=${value}`));
        } else {
