...
let mapCreator = {
    load: function (callback) {
        try {
            mapCreator = require(__dirname + '/lib/mapCreator')
            return true
        } catch (error) {
...
const Maphelper = function (options, adapter) {
    ...
    if(this.config.valetudo || this.config.mimap) mapCreator.load();

    adapter.log.debug("Maphelper_config___" + JSON.stringify(this.config));
    this.login();
};

Maphelper.prototype.getRawMapData = function (urlstring) {
    let self = this;
    let url;
...
Maphelper.prototype.getMapBase64 = function (url) {
    let self = this;
...
Maphelper.prototype.updateMap = function (mapurl, dontRetry) {
    let self = this;
    return new Promise(function (resolve, reject) {
