			...
			if (self.config.mimap === true) {
				...
				if (mapUrlCache[mapurl] == undefined || mapUrlCache[mapurl].expires < (unixtime - 60)) {
					...
					getMapURL.call(self, mapurl).then(function (result) {
						...
						if (!dontRetry) {
							self.login()
								.then(response => self.updateMap(mapurl, true))
								.catch(error => reject(error));
						}
					});
				} else {
		...
		request(options, (error, response, body) => {
			if (!error && response.statusCode === 200) {
				...
				if (json.message === 'ok') {
					...
					self.adapter.log.error('Error when receiving map url: ' + JSON.stringify(json));
					serviceToken = undefined;
					reject(json.message);
				}
				return;
			...
			try {
				if (error) {
					reject(error);
					self.adapter.log.warn('Get Error when receiving map url:' + error)
				} else {
					let json = JSON.parse(response.body);
