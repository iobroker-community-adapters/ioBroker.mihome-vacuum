...
const maptest = '["robomap%2F74476450%2F0"]';
	...
	getMapBase64 (url) {
		...
		return new Promise(function (resolve, reject) {
			...
			self.getRawMapData(url).then(function (data) {
				...
				const map = mapCreator.CanvasMap(data, self.config.COLOR_OPTIONS, self.adapter);
		...
		return new Promise(function (resolve, reject) {
			...
			request(options, function (error, response, body) {
				if (!error && response.statusCode === 200) {
					...
					self.adapter.log.debug('Login step1: '+ body);
					...
					request(options, function (error, response, body) {
						if (!error && response.statusCode === 200) {
							// Print out the response body
							self.adapter.log.debug('Login step2: '+ body);
							const jsonData = parseJson(body);
							...
							request(options, function (error, response, body) {
								if (!error && response.statusCode === 200) {
									...
									self.adapter.log.debug('Login step3: '+ JSON.stringify(cookies));
									let found = false;
									...
									if (error) self.adapter.log.error('HTTP error when getting token cookie' + JSON.stringify(error));
									else self.adapter.log.error('HTTP error when getting token cookie code: '+response.statusCode +' json_:'+ JSON.stringify(response));
		...
		return new Promise((resolve, reject) =>
			self.login(username, password)
				.then(resp => {
					...
					const body = generateSignature('/home/device_list', params);
...
function generateSignature(path, params) {
	...
	const signature = crypto.createHash('sha256').update(Buffer.concat([Buffer.from(ssecurity, 'base64'), Buffer.from(nonce, 'base64')])).digest('base64');
...
function getMapURL(mapName) {
	...
	return new Promise((resolve, reject) => {
		...
		const body = generateSignature('/home/getmapfileurl', params);
