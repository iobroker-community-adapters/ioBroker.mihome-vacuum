...
class Maphelper {
    constructor(options, adapter) {
        ...
        let did
        try {
            did = JSON.parse(adapter.config.MiDevice).did;
        } catch (error) {
            //adapter.log.error(error)
        }
        ...
        let requestWithEncoding = function (options, callback) {
            ...
            req.on('response', function (res) {
                ...
                if (res.statusCode !== 200){
                    if(res.statusCode === 404 ||res.statusCode === "404") callback('wrong server selected', null);
                    else callback('no map found on server', null);
                    return
                }
