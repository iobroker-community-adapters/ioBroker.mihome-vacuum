...
function Packet(token, adapter) {
    ...
    */
    ...
    this.getRaw_fast = function (plainData) {
        ...
        this.adapter.log.error("use old TS with: " + this.stamp);
        this.adapter.log.error("TS with ms:      " + (parseInt((new Date().getTime() + this.timediff2) / 1000)).toString(16));
        this.adapter.log.error("TS with s:       " + (parseInt((new Date().getTime()) / 1000) + this.timediff).toString(16));
