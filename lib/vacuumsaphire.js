...
const activeCleanStates = {
	5: {
		name: 'all ',
		resume: 'user.app_start'
	},
	11: {
		name: 'spot ',
		resume: 'user.app_spot'
	},
	17: {
		name: 'zone ',
		resume: 'user.app_resume_zoned_clean'
	},
	18: {
		name: 'segment ',
...
class VacuumManager {
	...
	async init() {
		...
		await Promise.all(objects.roomStates.map(async o => {
			const contents = await adapter.setObjectNotExistsAsync('info' + (o._id ? '.' + o._id : ''), o);
			contents && adapter.log.debug('Create State for Queue: ' + o._id);
		}));
		...
		if (adapter.config.enableMiMap || adapter.config.valetudo_enable) {
			...
			await Promise.all(objects.mapObjects.map(async o => {
				...
				contents && adapter.log.debug('Create State for map: ' + o._id);
			}));
		} else {
	...
	async getStates() {
		...
		if (!this.Error && this.features.carpetMode === null) {
	...
	async getOnlyAtStart(){
		for (const __fkt in this.startUp) {
			...
			let isTrue= await this[__fkt]();
			adapter.log.debug(`Startup: ${__fkt} Anser: ${isTrue}`);
	...
	async getMapPointer() {
		...
		try {
			for (let index = 0; index < 5; index++) {
				let answer = await this.Miio.sendMessage('user.get_map_v1');
		...
		this.miApi.updateMap(this.mapPointer)
			.then(async data => {
				if (data) {
					...
					const rooms = data[1];
					if ((this.device === 'roborock.vacuum.s5' || this.device === 'roborock.vacuum.s5e')&& this.features.roomMapping === false && typeof rooms !== 'undefined' && rooms.length > 0) {
						const roomids = [];
					...
					if (typeof zones !== 'undefined' && zones.length > 0 && (zones[0][0] !== this.lastZone[0][0])) {
					...
					const goto = data[3];
					if (typeof goto !== 'undefined' && goto.length > 0 && goto[0] !== this.lastGoto[0]) {
	...
	async checkFeaturesCarpet() {
		try {
			const answer = await this.Miio.sendMessage('user.get_carpet_mode');
	...
	async setGetCarpetMode() {
		try {
			const answer = await this.Miio.sendMessage('user.get_carpet_mode');
			if (answer.result[0].enable === 0 || answer.result[0].enable === 1) {
	...
	async setGetCleanSummary() {
		try {
			const summary = await this.parseCleaningSummary(await this.Miio.sendMessage('user.get_clean_summary'));
		...
		try {
			const start = async () => {
				await this.asyncForEach(logArray, async num => {
					const response = await this.Miio.sendMessage('user.get_clean_record', [num]);
	...
	async setGetSoundVolume() {
		try {
			const message = await this.Miio.sendMessage('user.get_sound_volume');
			this.Error = false;
	...
	async setGetConsumable() {
		...
		try {
			const message = await this.Miio.sendMessage('user.get_consumable');
	...
	async setGetStatus() {
		try {
			const status = await this.parseStatus(await this.Miio.sendMessage('app_get_status'));
			...
			if(status.map_status !== this.lastMapState){
				...
				await this.getMapPointer();
				await this.checkFeturesRoomMapping();
			}
				...
				case 'pauseResume':
					if (this.cleanActiveState) {
						if (this.cleandState === cleanStates.Pause) {
							...
							this.globalTimeouts['onMessage'] = setTimeout(this.setGetStatus, 2000);
							return await this.Miio.sendMessage(commands['resumeRoomClean'].method);
						}
					}
					break;
		...
		if (obj) {
			...
			switch (obj.command) {
				case 'sendCustomCommand':
					...
					// cleaning commands
				case 'startVacuuming':
					const answer = await this.Miio.sendMessage('user.app_start');
					this.globalTimeouts['onMessage'] = setTimeout(this.setGetStatus, 2000);
					return answer;

				case 'stopVacuuming':
					return await this.Miio.sendMessage('user.app_home');
				...
				case 'cleanSpot':
					if (this.startCleaning(cleanStates.SpotCleaning, obj)) {
						return await this.Miio.sendMessage('user.app_spot');
					}
					return;
				...
				case 'cleanZone':
					...
					if (this.startCleaning(cleanStates.ZoneCleaning, obj)) {
						return await this.Miio.sendMessage('user.app_zoned_clean', obj.zones);
					}
				...
				case 'pause':
					this.globalTimeouts['onMessage'] = setTimeout(this.setGetStatus, 2000);
					return this.Miio.sendMessage('user.app_pause');

				case 'charge':
					this.globalTimeouts['onMessage'] = setTimeout(this.setGetStatus, 2000);
					return this.Miio.sendMessage('user.app_home');

				case 'findMe':
					return await this.Miio.sendMessage('user.find_me');

				case 'getConsumableStatus':
					return await this.Miio.sendMessage('user.get_consumable');
				...
				case 'resetConsumables':
					...
					this.globalTimeouts['onMessage'] = setTimeout(this.setGetStatus, 2000);
					return await this.Miio.sendMessage('user.reset_consumable', obj.message.consumable);
				...
				case 'getCleaningRecord':
					...
					return await this.Miio.sendMessage('user.get_clean_record', [obj.message.recordId]);

					// TODO: find out how this works
					// case 'getCleaningRecordMap':
					//     sendCustomCommand('get_clean_record_map');
				case 'getMap':
					return await this.Miio.sendMessage('user.get_map_v1');

					// Basic information
				case 'getStatus':
					return await this.Miio.sendMessage('app_get_status');

				case 'getSerialNumber':
					return await this.Miio.sendMessage('user.get_serial_number');
				...
				case 'getDeviceDetails':
					...
					// Do not disturb
				case 'getDNDTimer':
					return await this.Miio.sendMessage('user.get_dnd_timer');
				...
				case 'setDNDTimer':
					...
					params = obj.message;
					return await this.Miio.sendMessage('user.set_dnd_timer', [params.startHour, params.startMinute, params.endHour, params.endMinute]);

				case 'deleteDNDTimer':
					return await this.Miio.sendMessage('user.close_dnd_timer');

					// Fan speed
				case 'getFanSpeed':
					return await this.Miio.sendMessage('user.get_custom_mode');
					//break;
				case 'setFanSpeed':
	...
	async setRemoteState(newVal) {
		...
		if (activeCleanStates[this.cleandState]) {
			...
			if ([cleanStates.Sleeping, cleanStates.Waiting, cleanStates.Back_toHome, cleanStates.Charging, cleanStates.GoingToSpot].indexOf(this.cleandState) > -1) {
	...
	async stopCleaning() {
		try {
			...
			await this.Miio.sendMessage('user.app_home');
			this.setGetStatus();
		} catch (error) {
...
class FeatureManager {
	...
	init() {
		...
		adapter.getState('info.device_fw', (err, state) =>
			state && state.val && this.setFirmware(state.val));
	...
	setFirmware(firmware) {
		...
		if (this.firmware !== firmware) {
			adapter.setStateChanged('info.fw_ver', firmware, true);
	...
	async setMop(mop_status) {
		if(typeof mop_status === 'undefined') return
