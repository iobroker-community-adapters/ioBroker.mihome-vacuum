...
class RoomManager {
        ...
        this.adapter.getChannelsOf('rooms', (_error, roomObjects) => {
            if (roomObjects) {
                for (const roomObject of roomObjects) {
                    const externalRoomId = roomObject._id.split('.').pop();
                    if (!externalRoomId.includes('manual_')) {
                        const room = rooms[externalRoomId];
                        if (!room) {
                            this.adapter.setStateChanged(`${roomObject._id}.mapIndex`, this.i18n.notAvailable, true, (_stateError, _id, notChanged) => {
                                if (!notChanged) {
                            ...
                            this.adapter.setStateChanged(`${roomObject._id}.mapIndex`, roomNumber, true, (_stateError, _id, notChanged) => {
                                if (!notChanged) {
                                    this.adapter.log.info(`room: ${externalRoomId} mapped with index ${roomNumber}`);
            ...
            for (const [stateId, state] of Object.entries(states)) {
                if (stateId.indexOf('.mapIndex') > 0) {
                    ...
                    if (!Number.isNaN(Number(value))) {
                        ...
                        this.adapter.log.error(`could not clean ${stateId}, because mapIndex/zone is invalid: ${value}`);
                    }
                }
                else {
    ...
    async updateRoomStates(roomObjectId) {
        ...
        try {
            ...
            await Promise.all(optionalRoomStates.map(async ([sourceId, targetId]) => {
