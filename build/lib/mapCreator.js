...
let canvasImg = (0, canvas_1.createCanvas)(100, 100);
//const ctximg = canvasImg.getContext('2d');
const img = new canvas_1.Image(); // Create a new Image
img.src = robot2;
const img_charger = new canvas_1.Image();
img_charger.src = charger;
function rotateCanvas(img, angle) {
    const canvasImg = (0, canvas_1.createCanvas)(100, 100);
    const ctxImg = canvasImg.getContext('2d');
    const offset = 90;
...
function CanvasMap(Mapdata, options, adapter) {
    ...
    let minleft = 0;
    let mintop = 0;
    const canvas = (0, canvas_1.createCanvas)(1024 * 4, 1024 * 4);
        ...
        if (typeof Mapdata.image.pixels.floor[0] === 'object') {
            ...
            ['floor', 'obstacle'].forEach(key => {
                ...
                Mapdata.image.pixels[key].forEach(function drawPixel(px) {
                    ...
                    ctx.rect((px % Mapdata.image.dimensions.width) * 4 + Mapdata.image.position.left * 4, (Mapdata.image.dimensions.height - 1 - Math.floor(px / Mapdata.image.dimensions.width)) * 4 +
                        Mapdata.image.position.top * 4, 4, 4);
                    maxtop = Math.max(maxtop, (Mapdata.image.dimensions.height - 1 - Math.floor(px / Mapdata.image.dimensions.width)) * 4 +
                        Mapdata.image.position.top * 4);
                    ...
                    mintop = Math.min(mintop, (Mapdata.image.dimensions.height - 1 - Math.floor(px / Mapdata.image.dimensions.width)) * 4 +
                        Mapdata.image.position.top * 4);
    ...
    if (Mapdata && Mapdata.image && Mapdata.image.pixels && Mapdata.image.pixels.carpet) {
        const offset = 65; // why the offset??
        ctx.beginPath();
        Mapdata.image.pixels.carpet.forEach(px => {
            ctx.strokeStyle = 'rgba(0,0,0,0.5)';
            ctx.lineWidth = 1;
            ctx.moveTo((px % Mapdata.image.dimensions.width) * 4 + Mapdata.image.position.left * 4 + offset, (Mapdata.image.dimensions.height - 1 - Math.floor(px / Mapdata.image.dimensions.width)) * 4 +
                Mapdata.image.position.top * 4);
            ctx.lineTo((px % Mapdata.image.dimensions.width) * 4 + Mapdata.image.position.left * 4 + offset + 4, (Mapdata.image.dimensions.height - 1 - Math.floor(px / Mapdata.image.dimensions.width)) * 4 +
                Mapdata.image.position.top * 4 +
                4);
        });
        ctx.stroke();
    ...
    if (Mapdata.robot) {
        ...
        if (Mapdata.path.current_angle &&
            typeof Mapdata.robot[0] !== 'undefined' &&
            typeof Mapdata.robot[1] !== 'undefined') {
            ctx.beginPath();
            canvasImg = rotateCanvas(img, Mapdata.path.current_angle);
    ...
    const canvas_final = (0, canvas_1.createCanvas)(finalWidth, finalHeight);
...
const MapCreator = Object.assign(() => undefined, { CanvasMap, rotateCanvas });
