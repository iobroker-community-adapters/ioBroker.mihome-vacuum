...
class Miio extends node_events_1.default {
    adapter;
    ownPort;
    port;
    ip;
    ...
    _finishRequest(messageCounter, settle, result) {
    ...
    _failRequest(messageCounter, code, message) {
        this._finishRequest(messageCounter, (request, error) => request.reject(error), this._createRequestError(code, message));
    }
    ...
    _handleMessage(message, remoteInfo) {
        ...
        this.adapter.log.debug(`MIIO request succeeded: method=${request.method}, id=${response.id}, duration=${Date.now() - request.startedAt}ms`);
        this._finishRequest(response.id, (pendingRequest, result) => pendingRequest.resolve(result), response);
    }
    __sendPing() {
        const checkAnswer = (message, remoteInfo) => {
            if (message.length === 32 && remoteInfo.port === Number.parseInt(String(this.port), 10)) {
                ...
                if (this.packet.timediff !== 0) {
                    this.adapter.log.debug(`Time difference between Mihome Vacuum and ioBroker: ${this.packet.timediff} sec`);
                }
        ...
        try {
            this.server.send(pingMessage, 0, pingMessage.length, Number.parseInt(String(this.port), 10), this.ip, error => {
                if (error) {
    ...
    async sendMessage(method, params) {
        ...
        return new Promise((resolve, reject) => {
            ...
            try {
                this.server.send(rawMessage, 0, rawMessage.length, Number.parseInt(String(this.port), 10), this.adapter.config.ip, error => {
                    if (!this.pendingRequests.has(messageCounter)) {
                        return;
                    }
                    if (error) {
                        this.adapter.log.debug(`MIIO request send failed: method=${safeMethod}, id=${messageCounter}, duration=${Date.now() - startedAt}ms`);
                        this._failRequest(messageCounter, 'MIIO_SEND_FAILED', 'MIIO request could not be sent');
                        return;
                    }
                    this.globalTimeouts[`sendMessage${messageCounter}`] = this.adapter.setTimeout(counter => {
                        this.adapter.log.debug(`MIIO request timed out: method=${safeMethod}, id=${String(counter)}, duration=${Date.now() - startedAt}ms, timeout=2000ms`);
    ...
    _buildMsg(method, params, messageCounter) {
        const message = {};
        if (method) {
...
class Packet {
    ...
    len = Buffer.alloc(2);
    unknown = Buffer.alloc(4);
    serial = Buffer.alloc(4);
    stamp = Buffer.alloc(4);
    checksum = Buffer.alloc(16);
    data = Buffer.alloc(0);
    token = Buffer.alloc(16);
    key = Buffer.alloc(16);
    iv = Buffer.alloc(16);
    ioskey = Buffer.from('00000000000000000000000000000000', 'hex');
    msgCounter = 1;
    stamprec = Buffer.alloc(4);
    timediff = 0;
    constructor(token) {
        this.setHelo();
        if (token) {
...
function decimalToHex(decimal, characters) {
