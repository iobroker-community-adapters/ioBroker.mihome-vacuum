...
class ViomiManager {
    ...
    async getStates() {
        ...
        if (deviceData && Array.isArray(deviceData.result)) {
            ...
            answer.slice(0, this.PARAMS.length).forEach((element, index) => {
                ...
                const objectExists = objects.viomiObjects.find((stateDefinition) => stateDefinition._id === parameter);
    ...
    async initStates() {
        await Promise.all(objects.viomiObjects.map((object) => this.adapter.setObjectNotExistsAsync(`control${object._id ? `.${object._id}` : ''}`, object)));
    }
