"use strict";
const rrMapHeader_1 = require("./rrMapHeader");
...
function PARSEBLOCK(buffer, offset, result = {}) {
    ...
    switch (type) {
        ...
        case TYPES.IMAGE: {
            ...
            if (image.dimensions.height > 0 && image.dimensions.width > 0) {
                const pixels = { floor: [], obstacle: [], segments: [] };
...
function PARSE(mapBuffer) {
    return (0, rrMapHeader_1.parseRRMapHeader)(mapBuffer);
}
function PARSEDATA(mapBuffer) {
    if (!PARSE(mapBuffer).map_index || !mapBuffer) {
        return null;
    }
    const blocks = PARSEBLOCK(mapBuffer, 0x14);
    const parsed = {};
    const image = getBlock(blocks, TYPES.IMAGE);
    if (!image) {
        return parsed;
    }
    parsed.image = image;
    image.pixels.carpet = getBlock(blocks, TYPES.CARPET_MAP);
    const paths = [
    ...
    for (const item of paths) {
        const path = getBlock(blocks, item.type);
        if (!path) {
            continue;
        }
        if (path.points) {
        ...
        if (item.name === 'mop_path') {
            parsed.mop_path = path;
        }
        else if (item.name === 'path') {
            parsed.path = path;
        }
        else {
            parsed.goto_predicted_path = path;
        }
    }
    const charger = getBlock(blocks, TYPES.CHARGER_LOCATION);
    if (charger) {
        parsed.charger = charger.position;
        parsed.charger[1] = DIMENSION_MM - parsed.charger[1];
    }
    const robot = getBlock(blocks, TYPES.ROBOT_POSITION);
    if (robot) {
        parsed.robot = robot.position;
        parsed.robot[1] = DIMENSION_MM - parsed.robot[1];
    }
    const gotoTarget = getBlock(blocks, TYPES.GOTO_TARGET);
    if (gotoTarget) {
        parsed.goto_target = gotoTarget.position;
        parsed.goto_target[1] = DIMENSION_MM - parsed.goto_target[1];
    }
    const cleanedZones = getBlock(blocks, TYPES.CURRENTLY_CLEANED_ZONES);
    if (cleanedZones) {
    ...
    const forbiddenZones = getBlock(blocks, TYPES.FORBIDDEN_ZONES);
    if (forbiddenZones) {
    ...
    const walls = getBlock(blocks, TYPES.VIRTUAL_WALLS);
    if (walls) {
    ...
    const cleanedBlocks = getBlock(blocks, TYPES.CURRENTLY_CLEANED_BLOCKS);
    if (cleanedBlocks) {
        parsed.currently_cleaned_blocks = cleanedBlocks;
    }
    return parsed;
}
const RRMapParser = Object.assign(function RRMapParser() { }, {
    TYPES,
    PARSEBLOCK,
    PARSE,
    PARSEDATA,
